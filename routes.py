...
refeicao = []  # Lista global para armazenar os valores da refeição
...
def index():
    logout()
    form = LoginForm()  # Instancie o formulário de login
    return render_template('login.html', form=form)

@app.route('/logout')
def logout():
    #session.pop('usuario_logado', None)  # Remove o ID do usuário da sessão
    session.clear()
...
def login():
    ...
    if form.validate_on_submit():  # Verifica se o formulário foi submetido e validado
        ...
        if usuario and check_password_hash(usuario.senha, form.senha.data):  # Verifica se o usuário existe e se a senha está correta
...
def balanca():
    ...
    refeicao_valor = [locale.currency(valor, grouping=True) for valor in refeicao]
...
def add_value():
    ...
    form = AddValueForm()  # Instancia o formulário Flask-WTF
    ...
    if form.validate_on_submit():
        if 'adicionar' in request.form:  # Se o botão "Adicionar" foi pressionado
            ...
            if refeicao:  # Verifica se há valores na lista de refeição
                ...
                balanca = Balanca(usuario_id=usuario_id, numero_refeicoes=numero_refeicoes, total_refeicoes=total)  # Cria uma instância da classe Balanca
                ...
                # Gere o PDF com os detalhes do registro de balança
        ...
        # Se o formulário não for válido ou não tiver sido submetido ainda, renderize a página balanca.html com o formulário
...
def caixa():
    ...
    if form.validate_on_submit():
        if 'buscar_id' in request.form:
            id_refeicao = form.id_refeicao.data
            session['id_refeicao'] = id_refeicao  # Armazena o ID na sessão
            registro_balanca = Balanca.query.filter_by(id=id_refeicao).first()
            if registro_balanca:
                total_pagamento = locale.currency(registro_balanca.total_refeicoes, grouping=True)
                flash(f'ID {id_refeicao} - Total a pagar: {total_pagamento}')  # 'success' é apenas um exemplo de categoria de flash message
                return redirect(url_for('caixa'))
            else:
                flash('ID não encontrado!')
                return redirect(url_for('caixa'))
        elif 'voltar_menu' in request.form:
            return redirect(url_for('menu'))
        elif 'realizar_pagamento' in request.form:
            id_refeicao = session.get('id_refeicao')  # Obtém o ID da sessão
            forma_pagamento = form.forma_pagamento.data
            registro_balanca = Balanca.query.filter_by(id=id_refeicao).first()
            if registro_balanca:
                registro_balanca.forma_pagamento = forma_pagamento
                db.session.commit()
